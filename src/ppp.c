                      ...
                      const nav_t *nav, double *dtrp, double *var)
{
    ...
    if (opt->tropopt == TROPOPT_SAAS) { /* Saastamoinen model */
        *dtrp = tropmodel(time, pos, azel, REL_HUMI);
        *var = SQR(ERR_SAAS);
        return 1;
    }
    if (opt->tropopt == TROPOPT_SBAS) { /* SBAS model */
        *dtrp = sbstropcorr(time, pos, azel, var);
        return 1;
    }
    if (opt->tropopt == TROPOPT_EST || opt->tropopt == TROPOPT_ESTG) { /* ZTD/ZTD+grad estimation */
    ...
    if (opt->tropopt == TROPOPT_VMF3 || opt->tropopt == TROPOPT_VMF3G) {
        ...
        if (fabs(timediff(grid1->time, grid2->time)) < 0.1) { // Menos de 0.1s de diferença
            ...
            // Calcular o fator de interpolação temporal (0.0 para T1, 1.0 para T2)
            double dt_total = timediff(grid2->time, grid1->time);
            double dt_interp = timediff(current_time_utc, grid1->time);
            double alpha_time = (dt_total == 0.0) ? 0.0 : dt_interp / dt_total;

            // Interpolar todos os coeficientes e ZTD/ZWD no tempo
            ...
            // MJD para calculate_vmf3_mapping_factors_C (usa o MJD do tempo atual)
            mjd_val = 51544.5 + timediff(current_time_utc, epoch2time((double[]){2000,1,1,12,0,0})) / 86400.0;
        }
