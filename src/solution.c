...
* 2016/07/30  1.15 suppress output if std is over sol_opt->maxsolstd
...
https://talk.newagtalk.com/forums/thread-view.asp?tid=877179&mid=7732209#M877179
Says "The industry pretty much standardized on using GPxxx regardless of what
constellations are being used in the solution.
To be completely NMEA compliant
the second letter should correspond to the constellations being used.
Don't
...
const prcopt_t *g_prc_opt_for_solution_output = NULL;
...
System used in RTK mode with fixed integers */
    /* 5=Float RTK.
Satellite system used in RTK mode, floating integers */
...
static const char *opt2sep(const solopt_t *sol_opt) /*  */
{
    if (!*sol_opt->sep) return " "; /*  */
    else if (!strcmp(sol_opt->sep,"\\t")) return "\t"; /*  */
    return sol_opt->sep; /*  */
}
/* separate fields -----------------------------------------------------------*/
static int tonum(char *buff, const char *sep, double *v)
{
    ...
    char *p,*q; // Adicionado: Declaração de p e q
...
static int decode_nmearmc(char **val, int n, sol_t *sol)
{
    ...
    for (i=0;i<n;i++) { /*  */
        switch (i) { /*  */
            case  0: tod =atof(val[i]); /*  */
            break; /* time in utc (hhmmss) */
            case  1: act =*val[i]; /*  */
            break; /* A=active,V=void */
            case  2: lat =atof(val[i]); /*  */
            break; /* latitude (ddmm.mmm) */
            case  3: ns  =*val[i]; /*  */
            break; /* N=north,S=south */
            case  4: lon =atof(val[i]); /*  */
            break; /* longitude (dddmm.mmm) */
            case  5: ew  =*val[i]; /*  */
            break; /* E=east,W=west */
            case  6: vel =atof(val[i]); /*  */
            break; /* speed (knots) */
            case  7: dir =atof(val[i]); /*  */
            break; /* track angle (deg) */
            case  8: date=atof(val[i]); /*  */
            break; /* date (ddmmyy) */
            case  9: ang =atof(val[i]); /*  */
            break; /* magnetic variation */
            case 10: mew =*val[i]; /*  */
            break; /* E=east,W=west */
            case 11: mode=*val[i];      break; /*  */
                                      /* mode indicator (>nmea 2) */
...
static int decode_nmeazda(char **val, int n, sol_t *sol)
{
    ...
    for (i=0;i<n;i++) { /*  */
        switch (i) { /*  */
            case  0: tod  =atof(val[i]); /*  */
            break; /* time in utc (hhmmss) */
            case  1: ep[2]=atof(val[i]); /*  */
            break; /* day (0-31) */
            case  2: ep[1]=atof(val[i]); /*  */
            break; /* mon (1-12) */
            case  3: ep[0]=atof(val[i]); /*  */
            break; /* year */
        }
    }
    septime(tod,ep+3,ep+4,ep+5); /*  */
...
static int decode_nmeagga(char **val, int n, sol_t *sol)
{
    ...
    for (i=0;i<n;i++) { /*  */
        switch (i) { /*  */
            ...
            case  1: lat =atof(val[i]); /*  */
            break; /* Latitude (ddmm.mmm) */
            case  2: ns  =*val[i]; /*  */
            break; /* N=north,S=south */
            case  3: lon =atof(val[i]); /*  */
            break; /* Longitude (dddmm.mmm) */
            case  4: ew  =*val[i]; /*  */
            break; /* E=east,W=west */
            case  5: solq=atoi(val[i]); break; /*  */
            /* GPS quality indicator */
            case  6: nrcv=atoi(val[i]); /*  */
            break; /* # of satellites in use */
            case  7: hdop=atof(val[i]); /*  */
            break; /* HDOP */
            case  8: alt =atof(val[i]); /*  */
            break; /* Altitude MSL */
            case  9: ua  =*val[i]; /*  */
            break; /* unit (M) */
            case 10: msl =atof(val[i]); /*  */
            break; /* Geoid separation */
            case 11: um  =*val[i]; /*  */
            break; /* unit (M) */
            case 12: age =atof(val[i]); /*  */
...
static int test_nmea(const char *buff)
{
    ...
    return !strncmp(buff+1,"GP",2)||!strncmp(buff+1,"GA",2)|| /* NMEA 4.10 [1] */
           ...
           !strncmp(buff+1,"GB",2)||!strncmp(buff+1,"GQ",2)|| /*  */
           /* NMEA 4.11 [2] */
           !strncmp(buff+1,"GI",2)|| /*  */
           !strncmp(buff+1,"BD",2)||!strncmp(buff+1,"QZ",2); /*  */
    /* extension */
}
...
static char *decode_soltime(char *buff, const solopt_t *sol_opt, gtime_t *time) // Use sol_opt
{
    double v[MAXFIELD]; /*  */
    char *p,*q,s[64]=" "; /*  */
    int n,len; /*  */
    ...
    if (!strcmp(sol_opt->sep,"\\t")) strcpy(s,"\t"); // Use sol_opt aqui /*  */
    else if (*sol_opt->sep) strcpy(s,sol_opt->sep);
    // Use sol_opt aqui /*  */
    len=(int)strlen(s); /*  */

    if (sol_opt->posf==SOLF_STAT) { // Use sol_opt aqui /*  */
        return buff; /*  */
    }
    if (sol_opt->posf==SOLF_GSIF) { /*  */
    ...
    /*%Y/%m/%d %H:%M:%S or %Y %m %d %H:%M:%S */
    if (sscanf(buff,"%lf/%lf/%lf %lf:%lf:%lf",v,v+1,v+2,v+3,v+4,v+5)>=6) { /*  */
        if (v[0]<100.0) { /*  */
            v[0]+=v[0]<80.0?2000.0:1900.0; /*  */
        }
        *time=epoch2time(v); /*  */
        if (sol_opt->times==TIMES_UTC) { /*  */
            *time=utc2gpst(*time); /*  */
        }
        else if (sol_opt->times==TIMES_JST) { /*  */
...
/* decode x/y/z-ecef ---------------------------------------------------------*/
extern int decode_solxyz(char *buff, const solopt_t *sol_opt, sol_t *sol) // Removido 'static'
{
    double val[MAXFIELD],P[9]={0}; /*  */
    int i=0,j,n; /*  */
    const char *sep=opt2sep(sol_opt); /*  */
...
extern int decode_solllh(char *buff, const solopt_t *sol_opt, sol_t *sol) // Removido 'static'
{
...
extern int decode_solenu(char *buff, const solopt_t *sol_opt, sol_t *sol) // Removido 'static'
{
...
static int decode_solsss(char *buff, sol_t *sol)
{
    ...
    for (i=0;i<6;i++) { /*  */
        ...
        sol->qr[i]=i<3?(double)SQR(std[i]):0.0f; /*  */
        sol->dtr[i]=0.0; /*  */
    }
    sol->ns=0; /*  */
...
static int decode_solgsi(char *buff, const solopt_t *sol_opt, sol_t *sol)
{
    double val[MAXFIELD]; /*  */
    int i=0,j; /*  */
...
extern int decode_solpos(char *buff, const solopt_t *sol_opt, sol_t *sol) // CORRIGIDO: Assinatura e sol_opt
{
    sol_t sol0={{0}}; /*  */
    char *p=buff; /*  */
    ...
    *sol=sol0; /*  */
    ...
    /* decode solution time */
    if (!(p=decode_soltime(p,sol_opt,&sol->time))) { // Use sol_opt /*  */
        return 0; /*  */
    }
    /* decode solution position */
    switch (sol_opt->posf) { // CORRIGIDO: sol_opt->posf /*  */
        case SOLF_XYZ : return decode_solxyz(p,sol_opt,sol); /*  */
        // Use sol_opt
        case SOLF_LLH : return decode_solllh(p,sol_opt,sol); /*  */
        // Use sol_opt
        case SOLF_ENU : return decode_solenu(p,sol_opt,sol); /*  */
        // Use sol_opt
        case SOLF_GSIF: return decode_solgsi(p,sol_opt,sol); /*  */
        // Use sol_opt
    }
    return 0; /*  */
}
/* decode reference position -------------------------------------------------*/
extern void decode_refpos(char *buff, const solopt_t *sol_opt, double *rb) // CORRIGIDO: Assinatura e sol_opt
{
    double val[MAXFIELD],pos[3]; /*  */
    int i,n; /*  */
    const char *sep=opt2sep(sol_opt); // Use sol_opt /*  */

    trace(3,"decode_refpos: buff=%s\n",buff); /*  */

    if ((n=tonum(buff,sep,val))<3) return; /*  */
    if (sol_opt->posf==SOLF_XYZ) { /* xyz */ // CORRIGIDO: sol_opt->posf /*  */
        for (i=0;i<3;i++) rb[i]=val[i]; /*  */
    }
    else if (sol_opt->degf==0) { /* lat/lon/hgt (ddd.ddd) */ // CORRIGIDO: sol_opt->degf /*  */
        pos[0]=val[0]*D2R; /*  */
        pos[1]=val[1]*D2R; /*  */
        pos[2]=val[2]; /*  */
        pos2ecef(pos,rb); /*  */
    }
    else if (sol_opt->degf==1&&n>=7) { /* lat/lon/hgt (ddd mm ss) */ // CORRIGIDO: sol_opt->degf /*  */
        pos[0]=dms2deg(val  )*D2R; /*  */
        pos[1]=dms2deg(val+3)*D2R; /*  */
        pos[2]=val[6]; /*  */
        pos2ecef(pos,rb); /*  */
...
/* decode solution -----------------------------------------------------------*/
extern int decode_sol(char *buff, const solopt_t *sol_opt, sol_t *sol, double *rb) // CORRIGIDO: Assinatura e sol_opt
{
    char *p; /*  */
    ...
    if (!strncmp(buff,COMMENTH,1)) { /* reference position */ /*  */
        ...
        decode_refpos(p+1,sol_opt,rb); // CORRIGIDO: Passando sol_opt /*  */
        return 0; /*  */
    }
    /* decode position record */
    return decode_solpos(buff,sol_opt,sol); /*  */
    // CORRIGIDO: Passando sol_opt
}
/* decode solution options ---------------------------------------------------*/
static void decode_solopt(char *buff, solopt_t *opt) // Mantenha 'opt' aqui
{
    char *p; /*  */
...
extern void readsolopt(FILE *fp, solopt_t *opt) // Mantenha 'opt' aqui, não renomeie
{
...
extern int inputsol(uint8_t data, gtime_t ts, gtime_t te, double tint,
                    int qflag, const solopt_t *sol_opt, solbuf_t *solbuf)
{
    sol_t sol={{0}}; /*  */
    int stat; /*  */
    ...
    /* decode solution */
    sol.time=solbuf->time; /*  */
    if ((stat=decode_sol((char *)solbuf->buff,sol_opt,&sol,solbuf->rb))>0) { // CORRIGIDO: opt para sol_opt /*  */
        if (stat) solbuf->time=sol.time; /*  */
        /* update current time */
        if (stat!=1) return 0; /*  */
    }
...
static int readsoldata(FILE *fp, gtime_t ts, gtime_t te, double tint, int qflag,
                       const solopt_t *sol_opt, solbuf_t *solbuf)
{
    int c; /*  */
    trace(3,"readsoldata:\n"); /*  */
    ...
    while ((c=fgetc(fp))!=EOF) { /*  */
        ...
        /* input solution */
        inputsol((uint8_t)c,ts,te,tint,qflag,sol_opt,solbuf); // CORRIGIDO: opt para sol_opt /*  */
    }
    return solbuf->n>0; /*  */
}
...
static int sort_solbuf(solbuf_t *solbuf)
{
    ...
    if (!(solbuf_data=(sol_t *)realloc(solbuf->data,sizeof(sol_t)*solbuf->n))) { /*  */
        ...
        free(solbuf->data); /*  */
        solbuf->data=NULL; solbuf->n=solbuf->nmax=0; /*  */
        return 0; /*  */
    }
...
static int sort_solstat(solstatbuf_t *statbuf)
{
    ...
    if (!(statbuf_data=realloc(statbuf->data,sizeof(solstat_t)*statbuf->n))) { /*  */
        ...
        free(statbuf->data); statbuf->data=NULL; /*  */
...
extern int outecef(uint8_t *buff, const char *s, const sol_t *sol,
                   const solopt_t *sol_opt) // CORRIGIDO: Assinatura e sol_opt
{
    const char *sep=opt2sep(sol_opt); /*  */
    // Use sol_opt
    char *p=(char *)buff; /*  */
    ...
    if (sol_opt->outvel) { /* output velocity */ // CORRIGIDO: sol_opt->outvel /*  */
...
extern int outpos(uint8_t *buff, const char *s, const sol_t *sol,
                  const solopt_t *sol_opt) // CORRIGIDO: Assinatura e sol_opt
{
    ...
    soltocov(sol,P); /*  */
    covenu(pos,P,Q); /*  */
    if (sol_opt->height==1) { /* geodetic height */ // CORRIGIDO: sol_opt->height /*  */
        pos[2]-=geoidh(pos); /*  */
    }
    if (sol_opt->degf) { // CORRIGIDO: sol_opt->degf /*  */
...
extern int outenu(uint8_t *buff, const char *s, const sol_t *sol,
                  const double *rb, const solopt_t *sol_opt) // CORRIGIDO: Assinatura e sol_opt
{
    ...
    if (sol_opt->outvel) { /* output velocity */ // CORRIGIDO: sol_opt->outvel /*  */
        p+=sprintf(p,"%s%10.5f%s%10.5f%s%10.5f%s%9.5f%s%8.5f%s%8.5f%s%8.5f%s"
                   "%8.5f%s%8.5f",
                   sep,enu[3],sep,enu[4],sep,enu[5],sep,SQRT(Q[0]),sep,
...
{
    // Adicionado 'static' para inicialização local dentro da função
    static const int sys[]={ /*  */
    ...
    "OFF","Saastamoinen","SBAS","Estimate ZTD","Estimate ZTD+Grad","VMF3 Model","VMF3 Model with Gradients","","" // Adicionados os rótulos VMF3
    };
...
* return : number of output bytes
*-----------------------------------------------------------------------------*/
extern int outsolheads(uint8_t *buff, const solopt_t *sol_opt, const prcopt_t *prc_opt) // Assinatura modificada
{
    const char *s1[]={"WGS84","Tokyo"},*s2[]={"ellipsoidal","geodetic"}; /*  */
    const char *s3[]={"GPST","UTC ","JST "},*sep=opt2sep(sol_opt); /*  */
    const char *leg1="Q=1:fix,2:float,3:sbas,4:dgps,5:single,6:ppp"; /*  */
    const char *leg2="ns=# of satellites"; /*  */
    char *p=(char *)buff; /*  */
    int timeu=sol_opt->timeu<0?0:(sol_opt->timeu>20?20:sol_opt->timeu); /*  */

    trace(3,"outsolheads:\n"); /*  */
    if (sol_opt->posf==SOLF_NMEA||sol_opt->posf==SOLF_STAT||sol_opt->posf==SOLF_GSIF) { /*  */
        return 0; /*  */
    }
    if (sol_opt->outhead) { // <--- Primeiro if (sol_opt->outhead) /*  */
        p+=sprintf(p,"%s (",COMMENTH); /*  */
        if      (sol_opt->posf==SOLF_XYZ) p+=sprintf(p,"x/y/z-ecef=WGS84"); /*  */
        else if (sol_opt->posf==SOLF_ENU) p+=sprintf(p,"e/n/u-baseline=WGS84"); /*  */
        else p+=sprintf(p,"lat/lon/height=%s/%s",s1[sol_opt->datum],s2[sol_opt->height]); /*  */
        p+=sprintf(p,",%s,%s)\r\n",leg1,leg2); /*  */
        // --- BLOCO CORRIGIDO E INTEGRADO ---
        const char *trop_options[] = { /*  */
            "OFF", "Saastamoinen", "SBAS", "Estimate ZTD", "Estimate ZTD+Grad",
            "VMF3 Model", "VMF3 Model with Gradients" // Corresponde aos seus defines 0-6
        }; /*  */
        p+=sprintf(p,"%s TROPOPT : %s\r\n",COMMENTH, trop_options[prc_opt->tropopt]); // Acesso a prc_opt->tropopt /*  */
        // --- FIM DO BLOCO CORRIGIDO ---

        p+=sprintf(p,"%s  %-*s%s",COMMENTH,(sol_opt->timef?16:8)+timeu+1,s3[sol_opt->times],sep); /*  */
    }

    if (sol_opt->posf==SOLF_LLH) { /* lat/lon/hgt */ // /* lat/lon/hgt */ /*  */
        if (sol_opt->degf) { /*  */
    ...
    else if (sol_opt->posf==SOLF_XYZ) { /* x/y/z-ecef */ /*  */
    ...
    else if (sol_opt->posf==SOLF_ENU) { /* e/n/u-baseline */ /*  */
        p+=sprintf(p,"%14s%s%14s%s%14s%s%3s%s%3s%s%8s%s%8s%s%8s%s%8s%s%8s%s%8s"
                   "%s%6s%s%6s",
...
extern int outsols(uint8_t *buff, const sol_t *sol, const double *rb,
                   const solopt_t *sol_opt, const prcopt_t *prc_opt) // Assinatura OK
{
    gtime_t time,ts={0}; /*  */
    double gpst; /*  */
    int week,timeu; /*  */
    const char *sep=opt2sep(sol_opt); // Use sol_opt aqui /*  */
    char s[64]; /*  */
    uint8_t *p=buff; /*  */

    trace(4,"outsols :\n"); /*  */
    /* suppress output if std is over sol_opt->maxsolstd */
    if (sol_opt->maxsolstd>0.0&&sol_std(sol)>sol_opt->maxsolstd) { // Use sol_opt aqui /*  */
        return 0; /*  */
    }
    if (sol_opt->posf==SOLF_NMEA) { // Use sol_opt aqui /*  */
        if (sol_opt->nmeaintv[0]<0.0) return 0; /*  */
        // Use sol_opt aqui
        if (!screent(sol->time,ts,ts,sol_opt->nmeaintv[0])) return 0; /*  */
        // Use sol_opt aqui
    }
    if (sol->stat<=SOLQ_NONE||(sol_opt->posf==SOLF_ENU&&norm(rb,3)<=0.0)) { // Use sol_opt aqui /*  */
        return 0; /*  */
    }
    timeu=sol_opt->timeu<0?0:(sol_opt->timeu>20?20:sol_opt->timeu); // Use sol_opt aqui /*  */

    time=sol->time; /*  */
    if (sol_opt->times>=TIMES_UTC) time=gpst2utc(time); /*  */
    // Use sol_opt aqui
    if (sol_opt->times==TIMES_JST) time=timeadd(time,9*3600.0); /*  */
    // Use sol_opt aqui

    if (sol_opt->timef) time2str(time,s,timeu); /*  */
    // Use sol_opt aqui
    else { /*  */
    ...
    switch (sol_opt->posf) { // Use sol_opt aqui /*  */
        case SOLF_LLH:  p+=outpos (p,s,sol,sol_opt); /*  */
        break; // Use sol_opt aqui
        case SOLF_XYZ:  p+=outecef(p,s,sol,sol_opt);   break; /*  */
        // Use sol_opt aqui
        case SOLF_ENU:  p+=outenu(p,s,sol,rb,sol_opt); break; /*  */
        // Use sol_opt aqui
              ...
              const solopt_t *sol_opt, const prcopt_t *prc_opt) // Assinatura OK
{
    ...
    if (sol_opt->posf==SOLF_NMEA) { // Use sol_opt aqui /*  */
        if (sol_opt->nmeaintv[1]<0.0) return 0; /*  */
        // Use sol_opt aqui
        if (!screent(sol->time,ts,ts,sol_opt->nmeaintv[1])) return 0; /*  */
        // Use sol_opt aqui
    }
    if (sol_opt->posf==SOLF_NMEA) { // Use sol_opt aqui /*  */
...
* return : none
*-----------------------------------------------------------------------------*/
extern void outsolhead(FILE *fp, const solopt_t *sol_opt)
{
    uint8_t buff[MAXSOLMSG+1]; /*  */
    int n; /*  */
    ...
    if ((n=outsolheads(buff, sol_opt, g_prc_opt_for_solution_output))>0) { // <--- CORRIGIDO
        fwrite(buff,n,1,fp); /*  */
    }
}
/* output solution body --------------------------------------------------------
       ...
       const solopt_t *sol_opt)
{
    ...
    if ((n=outsols(buff,sol,rb,sol_opt,g_prc_opt_for_solution_output))>0) { // <--- CORRIGIDO
        fwrite(buff,n,1,fp); /*  */
    }
}
/* output solution extended ----------------------------------------------------
                     ...
                     const solopt_t *sol_opt)
{
    ...
    if ((n=outsolexs(buff,sol,ssat,sol_opt,g_prc_opt_for_solution_output))>0) { // <--- CORRIGIDO
        fwrite(buff,n,1,fp); /*  */
    }	
}