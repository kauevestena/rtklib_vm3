...
static int resamb_LAMBDA(rtk_t *rtk, double *bias, double *xa,int gps,int glo,int sbs)
{
    ...
    if (!(info=lambda(nb,2,y,Qb,b,s))) {
        ...
        rtk->sol.ratio=s[0]>0?(double)(s[1]/s[0]):0.0f;
        ...
        if (opt->thresar[5]!=opt->thresar[6]) {
            ...
            rtk->sol.thres=(double)opt->thresar[0];
...
extern void rtkinit(rtk_t *rtk, const prcopt_t *opt)
{
    ...
    // --- CORREÇÃO: ADICIONE ESTA LINHA AQUI ---
    g_prc_opt_for_solution_output = opt; 
    // --- FIM DA CORREÇÃO ---

    rtk->initial_mode=rtk->opt.mode;
    rtk->sol.thres=(float)opt->thresar[0];

    // INÍCIO DA MODIFICAÇÃO: Chamar a função de inicialização do cache VMF3
    init_vmf3_cache(VMF3_OP_DIR); // 
